...
const PrimaryNameClass = enumType(["Name"]);
const Nick = stringType();
const Suffix = enumType(["", "II", "III", "IV", "Junior", "V"]);
const SurnameListClass = enumType(["Surname"]);
const TypeClass$1 = enumType([
    "EventRoleType",
    "NameOriginType",
    "NameType",
]);
const StringEnum$2 = enumType([
    "Also Known As",
    "Birth Name",
    "Bride",
    "",
...
const GedcomClass$1 = enumType(["Person"]);
const EventRefClass$1 = enumType(["EventRef"]);
enumType([
...
const AddressList = objectType({
    _class: stringType(),
    private: booleanType(),
    citation_list: arrayType(stringType()),
    note_list: arrayType(anyType()),
    date: nullType(),
    street: stringType(),
    locality: stringType(),
    city: stringType(),
...
const Type$2 = objectType({
    _class: TypeClass$1,
    string: StringEnum$2,
});
const EventRef$1 = objectType({
    _class: EventRefClass$1,
    private: booleanType(),
    note_list: arrayType(anyType()),
    attribute_list: arrayType(anyType()),
    ref: stringType(),
    role: Type$2,
});
const PersonRefList = objectType({
    _class: stringType(),
    private: booleanType(),
    citation_list: arrayType(anyType()),
    note_list: arrayType(anyType()),
    ref: stringType(),
    rel: stringType(),
});
const Surname = objectType({
    _class: SurnameListClass,
    surname: stringType(),
    prefix: stringType(),
    primary: booleanType(),
    origintype: Type$2,
    connector: stringType(),
});
const Name = objectType({
    _class: PrimaryNameClass,
    private: booleanType(),
    surname_list: arrayType(Surname),
    citation_list: arrayType(stringType()),
    note_list: arrayType(anyType()),
    date: nullType(),
    ...
    group_as: stringType(),
    sort_as: numberType(),
    display_as: numberType(),
    call: stringType(),
    nick: Nick,
    famnick: stringType(),
});
const GedcomElement = objectType({
    _class: GedcomClass$1,
    handle: stringType(),
    change: numberType(),
    private: booleanType(),
    tag_list: arrayType(stringType().min(20).max(30)),
    id: stringType(),
    citation_list: arrayType(stringType()),
    note_list: arrayType(stringType()),
    media_list: arrayType(anyType()),
    attribute_list: arrayType(anyType()),
    address_list: arrayType(AddressList),
    urls: arrayType(anyType()),
    lds_ord_list: arrayType(anyType()),
    primary_name: Name,
    event_ref_list: arrayType(EventRef$1),
    family_list: arrayType(stringType()),
    parent_family_list: arrayType(stringType()),
    alternate_names: arrayType(Name),
    person_ref_list: arrayType(PersonRefList),
    death_ref_index: numberType(),
    birth_ref_index: numberType(),
    gender: numberType(),
});

const ChildRefClass = z.enum(["ChildRef"]);
const PurpleClass = z.enum([
    "ChildRefType",
    "EventRoleType",
    "FamilyRelType",
]);
const StringEnum$1 = z.enum([
    "Birth",
    "Civil Union",
    "Family",
    "Foster",
...
const EventRefClass = z.enum(["EventRef"]);
const Type$1 = z.object({
    _class: PurpleClass,
    string: StringEnum$1,
});
...
const TypeClass = enumType(["AttributeType", "EventType"]);
const StringEnum = enumType([
    "Birth",
    "Death",
    "Education",
    "Elected",
...
const GedcomClass = enumType(["Event"]);
const DateClassEnum = enumType(["Date"]);
const Description = enumType(["", "Hogwarts"]);
const Place = enumType(["ed65c85d7b47cf245347468ab7d", ""]);
const Type = objectType({
    _class: TypeClass,
    string: StringEnum,
});
const DateClass = objectType({
    _class: DateClassEnum,
    format: nullType(),
    calendar: numberType(),
    modifier: numberType(),
    quality: numberType(),
    dateval: arrayType(unionType([booleanType(), numberType()])),
    text: stringType(),
    sortval: numberType(),
    newyear: numberType(),
});
...
objectType({
    ...
    note_list: arrayType(stringType()),
    media_list: arrayType(anyType()),
    attribute_list: arrayType(Attribute),
    date: unionType([DateClass, nullType()]),
    place: Place,
    type: Type,
...
async function handler(request) {
    ...
    const jsonFile = "../../../assets/gedcom/people.json";
    const peopleImport = (await import(jsonFile));
    const valid = GedcomElement.array().safeParse(peopleImport["default"]);
    if (valid.success) {
        if (DEBUG) {
    ...
    if (DEBUG) {
        console.log(`${FILENAME} has ${people ? people.length : 0} people, I will now look for id "${id}"`);
    }
    let body = {};
