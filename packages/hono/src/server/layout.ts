...
import { FrontMatter, ParsedResult, type NavigationItem } from "@schemas/page";
...
const createDirectoryIndex = (ast: Root, currentRoute?: string): void => {
  try {
    ...
    if (currentRoute) {
      ...
      const findRelevantSubtree = (items: NavigationItem[], route: string): NavigationItem[] => {
        // First check for exact match
        const exactMatch = items.find(item => item.route === route);
        if (exactMatch && exactMatch.children && exactMatch.children.length > 0) {
      ...
      const foundItems = findRelevantSubtree(sidebarRoutes.children, currentRoute);
      if (foundItems && foundItems.length > 0) {
        relevantItems = foundItems;
        if (DEBUG) {
          console.log(`Found relevant subtree with ${relevantItems.length} items`);
        }
      }
    }
...
const processHtml = async (
  ...
  try {
    ...
    if ("markdownContent" in options && options.markdownContent.includes("<directory-index>")) {
    ...
    } else if ("content" in options && options.content.includes("<directory-index>")) {
      hasDirectoryIndex = true;
      if (DEBUG) {
